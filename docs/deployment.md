...
|---|---|---|
| **Python library + CLI** | `memory_kg` package + unified `memorykg` CLI | PyPI, Conda, GitHub Releases |
| **Streamlit web app** | `memorykg viz` interactive graph explorer | Streamlit Cloud, Fly.io |
...
# Build sdist + wheel
poetry build
# → dist/memory_kg-0.1.0.tar.gz
# → dist/memory_kg-0.1.0-py3-none-any.whl
```
...
The `memorykg` CLI and all subcommands become available immediately:
...
1. Push the repo to GitHub (already done: `github.com/Flux-Frontiers/memory_kg`)
2. Go to [share.streamlit.io](https://share.streamlit.io) → **New app**
3. Select repo `Flux-Frontiers/memory_kg`, branch `main`, main file `src/memory_kg/app.py`
...
```bash
pip install https://github.com/Flux-Frontiers/memory_kg/releases/download/v0.1.0/memory_kg-0.1.0-py3-none-any.whl
```
...
memorykg mcp \
  --repo /path/to/repo \
  --db   /path/to/repo/.memorykg/graph.sqlite
```
...
```bash
poetry env info --path   # → /path/to/venv; binary at /path/to/venv/bin/memorykg
```
```json
{
  "mcpServers": {
    "memorykg": {
      ...
      "args": ["mcp", "--repo", "/abs/path", "--db", "/abs/path/.memorykg/graph.sqlite"]
    }
  }
}
```
...
- [ ] Test install in a fresh venv: `pip install dist/memory_kg-*.whl`
- [ ] Smoke-test all `memorykg` subcommands
