...
class AnalysisPipeline:
    ...
    def run(self, paths: list[Path] | None = None) -> PipelineResult:
        ...
        logger.info(
            "Phase 1: sampled %d / %d files", len(selected_paths), len(all_paths)
        )
    ...
    def _phase5_output(
        ...
        output_dir = cfg.output_dir or (
            Path(cfg.corpus_root) / ".memorykg" / "pipeline"
        )
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"# Unsupervised classifications: {method_counts['unsupervised']}\n"
            )
