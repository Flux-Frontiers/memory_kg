...
class DocKGSemanticAnalyzer:
    ...
    :param kg: An open :class:`~memory_kg.kg.DocKG` instance.
    ...
    def _write_report(self, report_path: str, result: dict) -> None:
        ...
        # ---- Dominant Themes ----
        readable_themes = [
            t for t in result["global_themes"] if not _ID_PATTERN.match(t["name"])
        ]
        lines += [
            "## Dominant Themes",
            "",
...
def _print_summary(console: Console, result: dict) -> None:
    ...
    table.add_row(
        "Avg sentence length", f"{lang.get('avg_sentence_length', 0.0):.1f} words"
    )
    ...
    readable = [
        t["name"]
        for t in result.get("global_themes", [])
        if not _ID_PATTERN.match(t["name"])
    ][:5]
