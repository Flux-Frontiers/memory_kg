...
_default_rels = "CONTAINS,NEXT,REFERENCES,SIMILAR_TO,HAS_TOPIC,MENTIONS_ENTITY,HAS_KEYWORD,CO_OCCURS_WITH"  # pylint: disable=invalid-name
...
def _get_kg() -> DocKG:
    ...
    if _kg is None:
        raise RuntimeError(
            "DocKG not initialised. Run via 'memorykg-mcp --repo /path/to/repo'"
        )
    return _kg
