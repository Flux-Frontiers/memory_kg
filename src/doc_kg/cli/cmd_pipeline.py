...
@click.option(
    "--batch-size", default=64, show_default=True, help="Per-worker batch size."
)
@click.option(
    "--sample-n", default=None, type=int, help="Evenly sample N texts before embedding."
)
@click.option(
    "--cache",
    default=None,
...
def pipeline_embed(
    ...
    cache_path = (
        Path(cache)
        if cache
        else (corpus_root / ".memorykg" / "pipeline" / "embeddings.json")
    )
