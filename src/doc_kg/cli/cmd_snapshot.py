...
from memory_kg.memorykg_thorough_analysis import DocKGAnalyzer
...
def save_snapshot(
    ...
    snapshots_path = (
        Path(snapshots_dir).resolve()
        if snapshots_dir
        else (repo_root / ".memorykg" / "snapshots")
    )
...
def list_snapshots(
    ...
    snapshots_path = (
        Path(snapshots_dir).resolve()
        if snapshots_dir
        else (Path.cwd() / ".memorykg" / "snapshots")
    )
    mgr = SnapshotManager(snapshots_path)
...
def prune_snapshots(snapshots_dir: str | None, dry_run: bool) -> None:
    ...
    if result.broken_entries:
        click.echo(
            f"{prefix}Broken manifest entries removed: {len(result.broken_entries)}"
        )
...
def diff_snapshots(
    key_a: str, key_b: str, snapshots_dir: str | None, output_json: bool
) -> None:
