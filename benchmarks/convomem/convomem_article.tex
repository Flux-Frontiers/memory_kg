...
MemoryKG is the only published system to achieve 100\% \emph{overall}
retrieval recall on ConvoMem (MemPal also reaches 100\% on Assistant Facts
at tier 1 specifically, but 92.9\% overall), and the only LLM-free system
to match or outperform long-context Gemini on the categories where
retrieval recall and QA accuracy are most correlated.
