...
### Added
- `src/memory_kg/query_denoise.py` + **opt-in `denoise=True` on `MemoryKG.query()` / `pack()`**: strips conversational distractor preamble from a query (keeping the trailing real question) before seeding — a safe no-op on clean queries. Productionizes the MemBench `noisy` lever (+32pp on that category). Also exposed as a `--denoise` flag on the MemBench harness; unit-tested in `tests/test_query_denoise.py`.
