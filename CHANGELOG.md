...
- **Stale `DOCKG_*` env vars renamed** to the `MEMORYKG_*` / shared-`KG`-ecosystem convention (memory_kg is no longer DocKG): `DOCKG_MODEL`→`MEMORYKG_MODEL` (`embedder_worker.py`), `DOCKG_DB`→`MEMORYKG_DB` and `DOCKG_LANCEDB`→`MEMORYKG_LANCEDB` (`app.py`), `DOCKG_SKIP_SNAPSHOT`→`MEMORYKG_SKIP_SNAPSHOT` (`cli/cmd_hooks.py`). Also corrected the `cli/cmd_model.py` docstring that referenced a nonexistent `DOCKG_MODEL_DIR` — the real model-cache override is the shared `KGRAG_MODEL_DIR`. Docs and `tests/test_embedder_worker.py` updated to match.

### Added
- `src/memory_kg/query_denoise.py` + **opt-in, default-off `denoise=True` on `MemoryKG.query()` / `pack()`** (and a `--denoise` probe flag on the MemBench harness): strips conversational distractor preamble from a query before seeding; a verified no-op on clean queries. **Framed as a diagnostic probe, not a product win** — its "last capitalized wh-clause" rule is a *benchmark-shaped heuristic* fitted to MemBench's synthetic `noisy` template, and its measured lift on that category (0.42→0.74) is reported only to show the gap is a **query-surface artifact, not a retrieval weakness**. Generalization is unproven; the lift must **not** be folded into headline MemBench numbers. Unit-tested in `tests/test_query_denoise.py`; rationale in `benchmarks/membench/ERROR_ANALYSIS.md`.
