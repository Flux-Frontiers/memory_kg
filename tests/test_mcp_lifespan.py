...
from memory_kg import MemoryKG, mcp_server
...
async def test_lifespan_closes_kg_on_server_shutdown(tmp_path: Path) -> None:
    # An unbuilt corpus is enough: stats() reports zeros off an empty schema
    # rather than raising, and reaching it is what opens the store.
    kg = MemoryKG(corpus_root=tmp_path, db_path=tmp_path / "graph.sqlite")
    mcp_server._kg = kg
    try:
